...
parser.add_argument('-i', '--jpg_path', type=str, default='/home/amit/experiment_comparing_model/AquaSAM/dataset/train_val/images', help='path to the jpg images')
parser.add_argument('-gt', '--bmp_path', type=str, default='/home/amit/experiment_comparing_model/AquaSAM/dataset/train_val/masks', help='path to the bmp images')
parser.add_argument('-o', '--npz_path', type=str, default='/home/amit/experiment_comparing_model/AquaSAM/dataset/npz_output', help='path to save the npz files')
...
parser.add_argument('--model_type', type=str, default='vit_l', help='model type')
parser.add_argument('--checkpoint', type=str, default='/home/amit/experiment_comparing_model/SAM_DEEPFISH_PTH/sam_l.pt', help='checkpoint')
...
# Get names from bmp path
names = sorted(os.listdir(args.bmp_path))
names = [f for f in names if f.endswith('.bmp')]
...
def preprocess_ct(bmp_path, jpg_path, bmp_name, image_name, label_id, image_size, sam_model, device):
    gt_pil = Image.open(join(bmp_path, bmp_name)).convert('L')  # read bmp image and convert to grayscale
    gt_data = np.array(gt_pil)
    
    # Handle different mask formats if necessary, assuming 255 is the object
    ...
    if np.sum(gt_data)>10:
        ...
        img_pil = Image.open(join(jpg_path, image_name))  # read jpg image
...
for name in tqdm(train_names):
    ...
    if not os.path.exists(join(args.jpg_path, image_name)):
